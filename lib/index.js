...
function cdf( x, options ) {
	...
	if ( isTypedArrayLike( x ) ) {
		...
		return cdf6( out, x, opts.v );
	}
	if ( isArrayLike( x ) ) {
		// Handle deepset first...
		if ( opts.path ) {
			opts.sep = opts.sep || '.';
			return cdf4( x, opts.v, opts.path, opts.sep );
		}
		...
		if ( opts.accessor ) {
			return cdf3( out, x, opts.v, opts.accessor );
		}
		return cdf2( out, x, opts.v );
	}
	return NaN;
} // end FUNCTION cdf()
